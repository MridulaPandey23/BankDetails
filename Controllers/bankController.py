...
def create_bank_details(bank: BankSchema, db: Session = Depends(get_db)):
    ...
    try:
        ...
        result = db.execute(query, {"user_id": bank.user_id, "acc_num": bank.acc_num, "bank_nm": bank.bank_nm})
        db.commit()
        row = result.fetchone()
        return api_response(
...
def get_bank(bank_id: int, db: Session = Depends(get_db)):
    try:
        ...
        result = db.execute(query, {"id": bank_id})
        row = result.fetchone()
...
def update_bank_details(bank_id: int, bank: BankSchema, db: Session = Depends(get_db)):
    existing = db.query(BankDetails).filter(BankDetails.id == bank_id).first()
    if not existing:
    ...
    try:
        ...
        result = db.execute(query, {"id": bank_id, "acc_num": bank.acc_num, "bank_nm": bank.bank_nm})
        db.commit()
        row = result.fetchone()
        return api_response(
            data={"id": row.id, "user_id": row.user_id, "acc_num": row.acc_num, "bank_nm": row.bank_nm},
